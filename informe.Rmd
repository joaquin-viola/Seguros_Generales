...
El objetivo de este trabajo es presentar distintos métodos para estimar los siniestros incurridos que se tendrán pasado los años de desarrollo. Y de esta manera obtener la reserva necesaria para hacer frente a lo que será reclemado.

Una de las principales herramientas del Chain-Ladder son los factores de desarrollo que se definiran más adelante. Se utilizan tanto en las matrices de pagos acumulados como en la de siniestros incurridos, y es el factor por el que los valores van creciendo de un período a otro. A veces se utiliza un solo factor de desarrollo para todo el período de desarrollo (ejemplo, para el crecimiento de los siniestros/pagos del período 6 al 7, sin importar el año de ocurrencia, lo que sería colomna por columna) y a veces es de particular interés los factores individuales, es decir, poder diferenciar tanto para el período de desarrollo como al año de ocurrencia (celda por celda).

En primer lugar se presentará el método de Chain-Ladder clásico, que utiliza la información del desarrollo de los reportes de los siniestros durante los años posteriores a que hayan ocurrido para predecir cómo se comportaran en el futuro, que en general, se suele asumir que luego de terminado el último período de desarrollo, no habrá nuevos reclamos y los costos no aumentarán. Luego, se presentará una variación que utiliza un modelo de regresión en función de los años transcurridos a partir de los factores de desarrollos observados y así obtener mediante estimaciones un factor de desarrollo mayor a 1 para el último período de desarrollo. Después se presentará el método de Mack Chain-Ladder, que bajo utiliza ciertos supuestos y obtiene una estimación de los desvíos de los siniestros a futuros y de la reserva de IBNR. Por último se presentará el método de Munich Chain-Ladder que utiliza un modelo de regresión y parte de la correlación existente entre los siniestros incurridos y los pagos acumulados.
...
Se define también, de manera iterativa, el factor de desarrollo acumulado. De la forma:

$$
Q_{j-1} = q_{j-1}\cdot Q_{j}
$$

Y en particular, para el último período de desarrollo se tiene, en general, que $Q_n = 1$. Luego, así como se puede predecir los siniestros incurridos para los valores por debajo de la diagonal de la manera:

$$
X_{i,c_i +1} = X_{i,c_i} \cdot q_{c_i}
$$

Se puede obtener con los factores de desarrollo acumulado el total a pagar y reservar por los siniestros ocurridos en el año $i$ ($X_i$) de la manera:

$$
X_i = Q_{c_i} \cdot X_{i,c_i}
$$

Que da el mismo resultado que hacer $X_i = X_{i,c_i} \cdot q_{c_i} \cdot \ldots \cdot q_{n}$, es decir, predecir todos los valores de la fila por debajo de la diagonal, y quedarnos con la última columna. Donde $c_i = n-i+1$ y queda definida la diagonal con todos los pares: $(i,c_i)$
...
print(xtable(Exhibit, type="latex", align="ccccc", digits = 3, caption=".", label = "exhibit"), caption.placement = "top", comment = FALSE)
```
...
print(xtable(factores_q1, type="latex", align="cc", digits = 3, caption=".", label = "factores1"), caption.placement = "top", comment = FALSE)
...
print(xtable(Exhibit2, type="latex", align="ccccc", digits = 3, caption=".", label = "exhibit2"), caption.placement = "top", comment = FALSE)
