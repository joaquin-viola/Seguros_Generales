...
En este trabajo se pretende abordar distintos métodos para el cálculo de reservas técnicas asociadas al IBNR. El IBNR por sus siglas en inglés, son los siniestros incurridos y que aún no fueron reclamados. En muchos países, los seguros de responsabilidad civil, cuentan con un período de 10 años para el reclamo de un seguro luego de que este haya ocurrido.

La idea principal de las reservas de IBNR es poder estar cubierto en el futuro de siniestros que puedan ocurrir en el año corriente (mientras está activa la poliza), pero el reclamo es efectualdo en los años siguientes.
...
```{r triangle, echo=FALSE, message=FALSE, warning=FALSE, caption = "\\label{triangle} Desarrollo de los reclamos por año y período."}

#plot(tri/1000, ylab="Siniestros en miles de pesos", xlab="Período de desarrollo")
